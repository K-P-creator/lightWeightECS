...
struct accelerationComponent
{
    float ax;
    float ay;
    accelerationComponent() : ax(0), ay(0) {}
    accelerationComponent(float x, float y): ax(x), ay(y) {}
...
{
    int r,g,b;
    outlineComponent(int R,int G,int B) : r(R), g(G), b(B) {}
    outlineComponent() : r(0), b(0), g(0) {} //defaults to black
...
struct textureComponent
{
    ...
    textureComponent(std::string filePath){
        if (!texture.loadFromFile(filePath)){
            std::cerr << "Unable to load texture at " << filePath << std::endl;
        }
    }
